...
EXAMPLES:
  ...
  02. Move a folder recursively from Hanzo S3 cloud storage to Amazon S3 cloud storage.
      {{.Prompt}} {{.HelpName}} --recursive play/mybucket/ s3/mybucket/

  03. Move multiple local folders recursively to Hanzo S3 cloud storage.
  ...
  05. Move files older than 7 days and 10 hours from Hanzo S3 cloud storage to Amazon S3 cloud storage.
      {{.Prompt}} {{.HelpName}} --older-than 7d10h play/mybucket/myfolder/ s3/mybucket/

  06. Move files newer than 7 days and 10 hours from Hanzo S3 cloud storage to a local path.
  ...
  09. Move a list of objects from local file system to Hanzo S3 cloud storage with specified metadata, separated by ";"
      {{.Prompt}} {{.HelpName}} --attr "key1=value1;key2=value2" Music/*.mp4 play/mybucket/

  10. Move a list of objects from local file system to Hanzo S3 cloud storage and set tags to the uploaded objects
      {{.Prompt}} {{.HelpName}} --tag "key1=value1" Music/*.mp4 play/mybucket/

  11. Move a folder recursively from Hanzo S3 cloud storage to Amazon S3 cloud storage with Cache-Control and custom metadata, separated by ";".
  ...
  15. Move a folder using client provided encryption keys from Amazon S3 to Hanzo S3 cloud storage.
      {{.Prompt}} {{.HelpName}} --r --enc-c "s3/documents/=MDEyMzQ1Njc4OTAxMjM0NTY3ODkwMTIzNDU2Nzg5MBB" --enc-c "mys3/documents/=MDEyMzQ1Njc4OTAxMjM0NTY3ODkwMTIzNDU2Nzg5MDA" s3/documents/ mys3/documents/

  16. Move a folder using specific server managed encryption keys from Amazon S3 to Hanzo S3 cloud storage.
      {{.Prompt}} {{.HelpName}} --r --enc-s3 "s3/documents" --enc-s3 "mys3/documents" s3/documents/ mys3/documents/

  17. Add SHA256 checksum to move a text file to Hanzo S3 cloud storage.
