...
var (
	cpFlags = []cli.Flag{
		...
		cli.BoolFlag{
			Name:  "zip",
			Usage: "Extract from remote zip file (Hanzo S3 server source only)",
		},
		cli.IntFlag{
			Name:  "max-workers",
...
EXAMPLES:
  ...
  02. Copy a folder recursively from Hanzo S3 cloud storage to Amazon S3 cloud storage.
      {{.Prompt}} {{.HelpName}} --recursive play/mybucket/myfolder/ s3/mybucket/

  03. Copy multiple local folders recursively to Hanzo S3 cloud storage.
  ...
  05. Copy files older than 7 days and 10 hours from Hanzo S3 cloud storage to Amazon S3 cloud storage.
      {{.Prompt}} {{.HelpName}} --older-than 7d10h play/mybucket/myfolder/ s3/mybucket/

  06. Copy files newer than 7 days and 10 hours from Hanzo S3 cloud storage to a local path.
  ...
  09. Copy a folder with encrypted objects recursively from Amazon S3 to Hanzo S3 cloud storage using s3 encryption.
      {{.Prompt}} {{.HelpName}} --recursive --enc-s3 "s3/documents" --enc-s3 "mys3/documents" s3/documents/ mys3/documents/

  10. Copy a folder with encrypted objects recursively from Amazon S3 to Hanzo S3 cloud storage.
      {{.Prompt}} {{.HelpName}} --recursive --enc-c "s3/documents/=MDEyMzQ1Njc4OTAxMjM0NTY3ODkwMTIzNDU2Nzg5MDA" --enc-c "mys3/documents/=MDEyMzQ1Njc4OTAxMjM0NTY3ODkwMTIzNDU2Nzg5BBB" s3/documents/ mys3/documents/

  11. Copy a list of objects from local file system to Hanzo S3 cloud storage with specified metadata, separated by ";"
      {{.Prompt}} {{.HelpName}} --attr "key1=value1;key2=value2" Music/*.mp4 play/mybucket/

  12. Copy a folder recursively from Hanzo S3 cloud storage to Amazon S3 cloud storage with Cache-Control and custom metadata, separated by ";".
